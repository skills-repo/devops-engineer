...
- 落地到 [`skills/ssh-deploy/SKILL.md`](../skills/ssh-deploy/SKILL.md)：rsync 同步、版本目录+软链接、systemd 托管、健康检查与回滚的具体写法与命令。
- 兄弟参考：
  - [`references/deployment-strategies.md`](deployment-strategies.md)：发布策略选型与回滚预案（SSH 部署也要先答"怎么回滚"）。
  - [`references/ci-cd-pipeline.md`](ci-cd-pipeline.md)：把 SSH 发布接进 CI 的部署阶段。
  - [`references/kubernetes.md`](kubernetes.md)：多实例/需自动扩缩容时改用 K8s，不再手写软链接。
  - [`references/scripts-usage.md`](scripts-usage.md)：SSH 部署前体检见 `dockerfile_lint.py`/`ci_audit.py`（镜像与流水线层面）。
